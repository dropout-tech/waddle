...
- PR73：https://github.com/dropout-tech/waddle/pull/73，已合併；PR68 關閉，由整合 PR73 接替。
- main SHA：ff4176f35f050345a4dad7784c0992d1867ee217。
- Zeabur deployment 6665353923：success，正式網址 https://waddle.zeabur.app。
- 正式 /widgets/ HTTP200，實際下載的 JS 包含新標題、11款、原生尚未提供安裝版提示及修正後專屬刷新事件。
- 正式瀏覽器驗收：390/1280px 預覽、種類篩選可操作、無水平溢出、無 pageerror。
- Cursor Security Agent：整合 HEAD 4cf79a0 success。
- 站內通知正式 RPC 200；直接讀表403、匿名401、讀取不存在通知403。舊寄信端點仍回403 invitation_unavailable，尚未部署410停用版本。
- 原本本機 mobile-widgets、scratchpad-free-canvas、meeting-availability-integration 分支提交均已推送；git log --branches --not --remotes 當時無剩餘提交。
- Google日曆、付款／推薦與帳號刪除草稿已在遠端 PR55；未開啟未完成服務。

**尚未完成**：排行榜正式 migration；send-meeting-invitations 410 no-op 部署。需使用者解鎖 Mac 恢復 Supabase 操作。排行榜遇到缺少 RPC 時隱藏，不宣稱排行榜已啟用。原生 App 仍無已驗收可安裝版。
