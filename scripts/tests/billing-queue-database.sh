...
psql -h "$BILLING_TEST_DIR" -d postgres -f "$(dirname "$0")/billing-queue-database.sql" >/dev/null
printf 'Billing database checks passed: migration, idempotency, stale snapshots, revocation, deleted user, RLS and service-only writes.\n'
