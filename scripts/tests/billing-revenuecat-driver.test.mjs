...
function fixture() {
  ...
  const packageObject = { identifier: '$rc_monthly', packageType: 'MONTHLY', product: { priceString: 'NT$149.00', title: 'Huddle Pro' } }
  const sdk = {
...
test('maps real SDK signatures, localized price, package object, restore and login', async () => {
  ...
  assert.deepEqual(await driver.listPackages(), [{ identifier: '$rc_monthly', localizedPrice: 'NT$149.00', title: 'Huddle Pro', period: 'monthly' }])
