...
const context = await browser.newContext({ locale: 'zh-TW', timezoneId: 'Asia/Taipei', viewport: { width: 1440, height: 1000 }, deviceScaleFactor: 1, serviceWorkers: 'block' })
...
await context.route('**/rest/v1/**', async route => {
 ...
 if (table === 'user_settings') return route.fulfill({ json: settings })
...
const capture = async filename => {
 ...
 assert.ok(!(await page.locator('body').innerText()).includes(env.E2E_EMAIL), 'Never capture the account email')
 await page.screenshot({ path: `public/marketing/${filename}`, animations: 'disabled' })
}
try {
 ...
 await page.getByText('完成首頁文案初稿', { exact: true }).first().waitFor({ timeout: 30000 })
 ...
 await page.getByText('今天想完成的事', { exact: true }).first().waitFor()
 ...
 await card.getByRole('button', { name: '開啟內容', exact: true }).click()
 await page.getByRole('dialog', { name: '白板內容', exact: true }).locator('.tiptap').waitFor()
