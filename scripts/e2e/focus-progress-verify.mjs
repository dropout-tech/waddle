...
try {
  ...
  const statusButton = card.getByRole('button', { name: '編輯「九豆」目前狀態' })
  await statusButton.click()
  const statusInput = card.getByRole('textbox', { name: '自訂狀態' })
  const beforeKeyboardWrites = writes.length
  await statusInput.fill('取消測試')
  await statusInput.press('Escape')
  check('Escape discards the draft without saving', writes.length === beforeKeyboardWrites && await statusButton.isVisible())
  await statusButton.click()
  await statusInput.fill('中文輸入測試')
  await statusInput.dispatchEvent('keydown', { key: 'Enter', code: 'Enter', isComposing: true, bubbles: true })
  check('IME Enter keeps the editor open without saving', writes.length === beforeKeyboardWrites && await statusInput.isVisible())
  await statusInput.press('Enter'); await sleep(500)
  check('Enter saves custom status once', storedFocus.cards[0].status?.text === '中文輸入測試' && writes.length === beforeKeyboardWrites + 1)
  await statusButton.click()
  ...
  check('Failed save leaves editor open for retry', await card.getByRole('button', { name: '重試', exact: true }).isVisible())
  failSave = false
  ...
  await mobileBoard.getByRole('heading', { name: '當前重點' }).click()
