...
try {
  ...
  // ── Status is display-only: chosen from the task list, never on the card ──
  check('No status picker on the card', await card.getByRole('button', { name: '編輯「九豆」目前狀態' }).count() === 0
    && await card.getByRole('button', { name: '編輯「九豆」狀態與備註' }).count() === 0
    && await card.getByRole('combobox', { name: '選擇狀態任務' }).count() === 0
    && await card.getByRole('textbox', { name: '自訂狀態' }).count() === 0)
  // ── Remarks are edited in place ──
  const remarksBox = card.getByRole('textbox', { name: '「九豆」備註' })
  check('Empty remarks show the placeholder in place', await remarksBox.getAttribute('placeholder') === '尚無備註' && await remarksBox.inputValue() === '')
  const restBox = await remarksBox.boundingBox()
  await remarksBox.click()
  const editBox = await remarksBox.boundingBox()
  const delta = ['x', 'y', 'width', 'height'].map((k) => Math.abs(restBox[k] - editBox[k]))
  check('Editing remarks keeps the same position and size (≤4px)', delta.every((d) => d <= 4), JSON.stringify({ restBox, editBox }))
  check('Inline remarks have no heavy border when focused', await remarksBox.evaluate((el) => { const cs = getComputedStyle(el); return parseFloat(cs.borderTopWidth) === 0 && (cs.outlineStyle === 'none' || parseFloat(cs.outlineWidth) === 0) }))
  check('Remarks editor adds no extra editor chrome', await card.locator('form').count() === 0 && await card.getByRole('button', { name: '取消', exact: true }).count() === 0)
  const beforeKeyboardWrites = writes.length
  await remarksBox.fill('取消測試')
  await remarksBox.press('Escape'); await sleep(200)
  check('Escape discards the draft without saving', writes.length === beforeKeyboardWrites && await remarksBox.inputValue() === '')
  await remarksBox.click()
  await remarksBox.fill('中文輸入測試')
  await remarksBox.dispatchEvent('keydown', { key: 'Enter', code: 'Enter', isComposing: true, bubbles: true })
  check('IME Enter keeps editing without saving', writes.length === beforeKeyboardWrites && await remarksBox.evaluate((el) => document.activeElement === el))
  await remarksBox.fill('週五追蹤報價\n尚未建立任務')
  await board.getByRole('heading', { name: '當前重點' }).click(); await sleep(500)
  check('Blur saves multiline remarks once', storedFocus.cards[0].remarks === '週五追蹤報價\n尚未建立任務' && writes.length === beforeKeyboardWrites + 1)
  await remarksBox.click()
  await board.getByRole('heading', { name: '當前重點' }).click(); await sleep(200)
  check('Unchanged blur avoids another write', writes.length === beforeKeyboardWrites + 1)
  ...
  check('Legacy status preserved read-only', await card.getByText('推動金流物流', { exact: true }).isVisible())
  check('Remarks save creates no task', !writes.some((w) => w.table === 'tasks' && w.method === 'POST'))
  await page.reload(); await sleep(2000); await openBoard(page)
  check('Remarks survive normalization and reload', await card.getByRole('textbox', { name: '「九豆」備註' }).inputValue() === '週五追蹤報價\n尚未建立任務')
  failCreate = true
  await card.getByRole('button', { name: '將此狀態新增為任務' }).click(); await sleep(600)
  check('Failed conversion keeps legacy status and allows retry', await card.getByRole('button', { name: '將此狀態新增為任務' }).isEnabled() && await card.getByText('推動金流物流', { exact: true }).isVisible())
  failCreate = false
  await card.getByRole('button', { name: '將此狀態新增為任務' }).click(); await sleep(600)
  ...
  await card.getByRole('button', { name: '取消「談好宅配的到府取件費率」目前狀態' }).click(); await sleep(400)
  check('Tapping the active ◎ clears the current status', storedFocus.cards[0].status?.mode === 'off' && await card.getByRole('button', { name: '將「談好宅配的到府取件費率」設為目前狀態' }).isVisible())
  ...
  const remarks2 = card.getByRole('textbox', { name: '「九豆」備註' })
  await remarks2.fill('儲存失敗測試')
  failSave = true
  await board.getByRole('heading', { name: '當前重點' }).click(); await sleep(500)
  check('Failed save keeps the draft and offers retry', await card.getByRole('button', { name: '重試', exact: true }).isVisible() && await remarks2.inputValue() === '儲存失敗測試')
  failSave = false
  await card.getByRole('button', { name: '重試', exact: true }).click(); await sleep(400)
  check('Retry saves the remarks', storedFocus.cards.find((c) => c.categoryId === CATS[0].id).remarks === '儲存失敗測試' && await card.getByRole('button', { name: '重試', exact: true }).count() === 0)
  await card.getByRole('button', { name: `將「${taskRows[0].title}」設為目前狀態` }).click(); await sleep(400)
  ...
  const mobileRemarks = mobileCard.getByRole('textbox', { name: '「Nova air」備註' })
  // Compare against the card itself: tapping scrolls the board (keyboard
  // avoidance), so absolute page coordinates are expected to move.
  const relBox = () => mobileRemarks.evaluate((el) => { const r = el.getBoundingClientRect(); const c = el.closest('article').getBoundingClientRect(); return { x: r.x - c.x, y: r.y - c.y, width: r.width, height: r.height } })
  await mobileRemarks.scrollIntoViewIfNeeded()
  const mRest = await relBox()
  await mobileRemarks.click()
  const mEdit = await relBox()
  await sleep(700)
  check('Focused remarks are scrolled into view', await mobileRemarks.evaluate((el) => { const r = el.getBoundingClientRect(); return r.top >= 0 && r.bottom <= innerHeight }))
  check('Mobile: editing remarks keeps position and size within the card (≤4px)', ['x', 'y', 'width', 'height'].every((k) => Math.abs(mRest[k] - mEdit[k]) <= 4), JSON.stringify({ mRest, mEdit }))
  const longRemarks = '需要核對的事項\n' + 'https://example.com/' + 'long-path-'.repeat(24)
  await mobileRemarks.fill(longRemarks)
  ...
  check('Remarks grow with content instead of scrolling inside', await mobileRemarks.evaluate((el) => el.scrollHeight <= el.clientHeight + 2))
  await page.setViewportSize({ width: 320, height: 420 })
  await mobileBoard.getByRole('heading', { name: '當前重點' }).click(); await sleep(500)
  check('Remarks save with reduced keyboard-like height', storedFocus.cards.find((c) => c.categoryId === CATS[1].id).remarks === longRemarks)
  await page.setViewportSize({ width: 320, height: 640 })
  ...
  await sleep(400)
  const lastTaskButton = mobileBoard.locator('article').last().getByRole('button', { name: /目前狀態/ }).last()
