...
const locale = process.argv.includes('--lang=en') ? 'en' : 'zh'
...
try {
  ...
  await page.goto(base + `/marketing/feature-film/index.html?lang=${locale}`, { waitUntil: 'networkidle' })
  ...
  encoder = spawn('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'warning', '-f', 'image2pipe', '-vcodec', 'mjpeg', '-framerate', String(fps), '-i', 'pipe:0', '-i', resolve(out, 'huddle-score.wav'), '-map', '0:v:0', '-map', '1:a:0', '-vf', 'pad=1920:1480:0:0:black', '-c:v', 'libx264', '-preset', 'medium', '-crf', '18', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k', '-ar', '48000', '-t', String(duration), '-movflags', '+faststart', resolve(out, `huddle-feature-film${suffix}.mp4`)], { stdio: ['pipe', 'inherit', 'inherit'] })
  let encoderError
  ...
  for (const time of [0, 2.25, 4.875, 6.75, 9, 11.25, 13.5, 15.75, 17.625]) {
    ...
    const target = resolve(`/tmp/huddle-film-review-${locale}`, `frame-${time}.png`)
  ...
  const posterEncoder = spawn('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-i', 'pipe:0', '-vf', 'pad=1920:1480:0:0:black', '-frames:v', '1', resolve(out, `poster${suffix}.jpg`)], { stdio: ['pipe', 'inherit', 'inherit'] })
