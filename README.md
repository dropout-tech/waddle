...
一個整合 **任務管理 + 時間區塊排程 + 日記反思** 的工作面板，三欄式響應式應用。標語集中在 `lib/brand.ts`。
...
ScratchpadItem     # 白板卡片（text/image/link/todo），依 date 分組、雲端同步；metadata.canvas 保存自由畫布位置與大小
```
...
- 白板（scratchpad）走 `scratchpad_items` 雲端表（block 模型 schema 見 migration `0011`）。上方快速卡片與下方每日自由畫布共用同一筆資料與 ID；卡片進入畫布時只於 `metadata.canvas` 保存 `x/y/width/height`，移回上方時清除這段 metadata，不複製內容。手寫筆跡以圖片卡片儲存，也能移動與調整大小。所有新增、編輯、排序、刪除與清空寫入依序處理，避免位置或新卡片被舊請求覆蓋。首次載入會自動把舊 localStorage `scratchpad-YYYY-MM-DD` 鍵搬遷上去（一次性，記在 `waddle-scratchpad-migrated-v1` flag）。常用連結用 upsert 寫 `user_settings.quick_links`，避免 row 缺失時 silently 0-rows。
...
自由畫布回歸：`node scripts/e2e/scratchpad-canvas-verify.mjs`（同樣將 `scratchpad_items` 的所有寫入攔截為 mock），驗證原卡片保留、同 ID 移入／移回、拖曳與縮放只在操作結束寫入、失敗回滾、重新整理還原、手寫與 320/390/430 px 手機寬度。
