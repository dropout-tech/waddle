...
export default function MeetingsPage() {
  ...
  const { lang } = useI18n()
  const en = lang === 'en'
  const { peers } = useCalendarSharing(!!user)
  const [inviteId, setInviteId] = useState<string>()
  const controller = useMeetingInvitations(undefined, inviteId)
  useEffect(() => {
    ...
    try {
      if (!user && !loading)
        sessionStorage.setItem('huddle-pending-meeting-invite', id)
      if (user) sessionStorage.removeItem('huddle-pending-meeting-invite')
    } catch {
      /* The invitation remains readable even when browser storage is blocked. */
    }
  }, [user, loading])
  ...
  if (!user)
    return (
      <main className="mx-auto max-w-xl space-y-4 p-8">
        <h1 className="text-2xl">{en ? 'Meeting invitation' : '會議邀請'}</h1>
        <p>
          {en
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-5 p-5 sm:p-8">
      <Link href="/" className="text-sm underline">
        {en ? 'Back to calendar' : '返回行事曆'}
      </Link>
      <h1 className="text-2xl font-semibold">
        {en ? 'Meeting invitations' : '會議邀請'}
      </h1>
      <p className="text-sm text-muted-foreground">
        {en
          ? 'Common times reflect currently shared calendars, not a guaranteed reservation.'
          : '共同空檔僅依目前共享行程判斷，不代表已保留時段。'}
      </p>
      <MeetingPanel
