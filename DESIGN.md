...
此為既有 warm ceramic 工作介面的延伸，範圍限 `/widgets/`；不改寫上方公開官網海報或 App 設計系統。方向為「把今天，放在手邊」：將月曆、任務與想法呈現在各自獨立的小工具中，讓使用者選擇搭配，保持溫柔、不催促的語氣。

- **品牌與色彩**：角色只能直接使用 `public/huddle-mascot.png` 原圖，原生資產副本也必須保持相同內容；示意圖的重繪角色不得替代正式素材。沿用暖米底、赤陶主要動作與鼠尾草次要動作，以及既有語意 token；不套入 marketing 黃色海報風格。樣式來源為 `components/widgets/widgets.module.css`，方向契約位於 `components/widgets/widget-gallery.tsx`。
- **11 款內容**：月曆＋今日任務、可視化小月曆、近期行程、任務清單、今天三件事、白板、記事本、專注記事、專注計時、喝水提醒、三合一隨手記入口。內容類型與主要操作保持可辨識，不把所有小工具簡化為同一種數字卡片。
- **響應式網頁**：容器最大 1240px，桌面三欄、1000px 以下兩欄、640px 以下單欄；欄距 24px，手機 18px。小工具採 24px 圓角、20px 內距與極淺暖炭陰影。頁首標題 26–38px，卡片標題 15px，內文沿用 App 字體與語意色。尺寸選擇提供小／中／大；目前 small 限寬 280px，medium 與 large 共用版面，不能視為三種原生尺寸的精確模型。
- **互動與狀態**：示範資料與登入後資料有明確提示；白板與記事編輯沿用既有編輯器。控制項通常至少 44px，鍵盤焦點使用 ring outline；目前小尺寸月曆日期格為 32px，是仍需改善的觸控例外。完成狀態以刪除線及控制項狀態呈現，不只靠顏色。
