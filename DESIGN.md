...
- 每個分類採同等層級的標題，依序呈現「目前狀態」、「備註」與完整任務列；不以急迫／停滯分層或縮成前三筆摘要。任務列可換行，使用細分隔線，不再套內層卡片。
- 桌面與手機共用 `components/task-panel/focus-board.tsx`；手機由 `focus-board-mobile.tsx` 提供返回與捲動容器。主要操作、排序選單及任務列觸控目標至少 44px。
