...
- **魚游標**：整個行銷頁（`<main>` 掛 `fishZoneClass`），只在 `(hover: hover) and (pointer: fine)` 生效；連結／按鈕／summary 用 pointer、輸入框用 text。
- **點企鵝＝餵魚**：魚從點擊處拋物線飛進張開的嘴（滑鼠也可點企鵝附近 280px 內的空白處；UI 元素永遠不觸發）。每條魚變胖一點，第 4 條坐下吃飽再小睡，之後慢慢瘦回來。約 25% 機率改成其他荒謬反應（分身、丟卡、嘎、滑倒、墨鏡、泡泡嚇到）。
- **閒置情緒**：停在靜止停靠點 4.5–9 秒後隨機打哈欠／偷看手機／揮旗／墨鏡／滑倒爬起／被泡泡嚇；每個停靠點每次到站從 `VARIANTS` 隨機換姿勢。
- **神奇玩具**（`components/marketing/magic-toys.tsx`）：`<ToyButton toy>` 丟事件、`<ToyStage toy>` 放在該區插畫內播放。任務（雜事掉落→疊好→蓋章）、行事曆（時間塊吸進週格）、會議（亂碼旋轉→三張卡飛進手機）、專注（通知被泡泡罩彈開＋環跑一圈＋煙火）、FAQ 旁每日簽到（越按越誇張）。玩具是功能的延伸想像，不是功能本身；舞台 aria-hidden、按鈕有 role=status 文字回報。reduced-motion：每個動作直接跳到結果並停留，不動。
