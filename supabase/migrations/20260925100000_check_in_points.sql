...
create table public.points_accounts (
  ...
  ranking_points bigint not null default 0 check (ranking_points >= 0),
  updated_at timestamptz not null default now()
);
create index points_accounts_ranking_idx on public.points_accounts (ranking_points desc, user_id);

create table public.points_ledger (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  kind text not null check (kind in ('daily_check_in', 'redemption', 'refund', 'adjustment')),
  source_key text not null check (length(source_key) between 1 and 200),
  points_delta bigint not null check (points_delta <> 0),
  ranking_delta bigint not null default 0,
  check_in_date date,
  ...
  check ((kind = 'daily_check_in') = (check_in_date is not null)),
  check (kind <> 'daily_check_in' or (points_delta > 0 and ranking_delta = points_delta)),
  check (kind <> 'redemption' or (points_delta < 0 and ranking_delta = 0)),
  check (kind <> 'refund' or (points_delta > 0 and ranking_delta = 0))
);
...
-- Future trusted service operations can append ledger entries, never overwrite them.
...
-- The atomic UPDATE locks the account row. A negative balance rejects the entire
-- ledger insertion, including concurrent future redemptions.
...
begin
  ...
  update public.points_accounts set
    available_points = available_points + new.points_delta,
    ranking_points = ranking_points + new.ranking_delta,
    updated_at = now()
...
returns table(check_in_date date, checked_in boolean, available_points bigint, ranking_points bigint, daily_points integer)
language sql stable security invoker set search_path = '' as $$
  select
    ...
    coalesce(a.available_points, 0), coalesce(a.ranking_points, 0), 1
...
begin
  ...
  insert into public.points_ledger(user_id, kind, source_key, points_delta, ranking_delta, check_in_date, description)
    values (v_user, 'daily_check_in', 'daily_check_in:' || v_day::text, 1, 1, v_day, '每日簽到')
...
comment on table public.points_ledger is 'Append-only point events. Redemption/refund change spendable points only; adjustments explicitly state ranking impact. No historical automatic backfill.';
comment on table public.points_accounts is 'Transactional ledger projection. ranking_points survives redemption; indexed for future ranking, not publicly exposed.';
commit;
