          ...
          <section className={styles.panel}>
            ...
            {data.paid_until && (
              <p className={styles.muted}>
                {t('付費權益至 {date}', { date: dateLabel(data.paid_until) })}
                {t('。贈送時間接在付費權益之後；商店自動續訂與扣款仍依原訂閱設定，請在購買商店管理。')}
              </p>
            )}
          </section>
          <div className={styles.grid}>
            <section className={styles.panel}>
              ...
              <p className={styles.muted}>
                {data.settings.referrals_enabled
                  ? t('新朋友在註冊後 7 天內使用並完成帳號驗證，你就能獲得 {days} 天。每年最多 {cap} 天。', {
                      days: data.settings.referral_days,
                      cap: data.settings.annual_reward_cap,
                    })
              ...
              <p className={`${styles.muted} mt-5`}>
                {t('已成功推薦 {count} 人 · 累積獲得 {days} 天', {
                  count: data.referral_count,
                  days: data.reward_days,
                })}
              </p>
            </section>
            <section className={styles.panel}>
            ...
            <section className={styles.panel}>
              ...
              {data.referred ? (
                ...
                <>
                  <p className={styles.muted}>
                    {t('註冊後 7 天內可補填一次。新戶推薦體驗總共 {days} 天，包含已領取的新戶體驗；活動優惠擇優，不重複贈送。', {
                      days: data.settings.friend_days,
                    })}
                  </p>
                  <form
                    className="mt-5 space-y-4"
