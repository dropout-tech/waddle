...
function Inbox({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => {
        if (active.current) setError("無法讀取分類，請重新開啟總覽。");
      });
    const load = () => {
      if (document.visibilityState === "visible")
        void refresh().catch(() => {
          if (active.current) setError("暫時無法讀取指派，請稍後重新整理。");
        });
    };
    load();
  ...
  async function respond(item: MeetingAssignment, accept: boolean) {
    ...
    try {
      ...
      if (active.current)
        setError(e instanceof Error ? e.message : "未能處理指派");
    } finally {
      lock.current = false;
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-medium">
          待接受指派{pending.length > 0 ? ` · ${pending.length}` : ""}
        </h2>
        <button
          type="button"
          disabled={busy}
