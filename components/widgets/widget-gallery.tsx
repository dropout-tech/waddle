...
function GalleryFrame({data,onOpen,onComplete,onAction,live=false,onMode,children}:{data:WidgetSnapshot;onOpen:(kind:WidgetKind,id?:string,date?:string)=>void;onComplete:(id:string)=>void;onAction:(action:string)=>void;live?:boolean;onMode?:()=>void;children?:React.ReactNode}) {
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.heading}><HuddleMascot className="h-20 w-20"/><div><h1>把今天，放在手邊</h1><p>看一眼安排，勾掉一件事，留下一個想法。用你喜歡的方式，搭配自己的 Huddle 小工具。</p></div></div>
    ...
    {children}
    <div className={styles.grid}>{widgetKinds.filter(k=>filter==='all'||filter===k).map(kind=><WidgetCard key={kind} kind={kind} data={data} size={size} onOpen={onOpen} onComplete={onComplete} onAction={onAction}/>)}</div>
