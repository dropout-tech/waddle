  ...
  useEffect(()=>{
    ...
    const sync=async()=>{
      ...
      try {
        ...
        if(!alive || widgetAccount().accountId!==source) return
        ...
        snapshot.focus={state:x.timer.state,title:s?.label ?? '慢慢來，先專心一件事',seconds:x.timer.displayTime,endAt:s && x.timer.state==='running' && s.mode==='pomodoro' ? s.startedAt.getTime()+s.pausedMs+s.targetSeconds*1000:null,note:focusNoteExcerpt(x.notes,snapshot.today,s?.label)}
    ...
    return ()=>{alive=false;clearInterval(id);clearTimeout(first);window.removeEventListener('focus',onVisible);document.removeEventListener('visibilitychange',onVisible)}
  },[user?.id])
  useEffect(()=>{window.dispatchEvent(new Event('focus'))},[workspaces,timeBlocks,boards,notebook.notes,notes,timer.state,timer.session])
  return null
}
